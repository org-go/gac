...
type TagCallBack func(columnName string, isPrimaryKey bool) string
...
type Table2Struct struct {
	...
	prefix               string
	config               *T2tConfig
	tagCallBacks         []TagCallBack
	err                  error
	realNameMethod       string
	clickHouseNameMethod string
...
func (t *Table2Struct) ClickHouseNameMethod(r string) *Table2Struct {
	t.clickHouseNameMethod = r
	return t
}
...
func (t *Table2Struct) TagCallBack(cb TagCallBack) *Table2Struct {
	t.tagCallBacks = append(t.tagCallBacks, cb)
	return t
}
...
func (t *Table2Struct) Run() error {
	...
	if t.config.SeperatFile && !f.IsDir() {
		return errors.New("can't use separate mod with file")
	}
	if !t.config.SeperatFile && f.IsDir() {
		return errors.New("can't use file mod with directory")
	}
	...
	for tableRealName, item := range tableColumns {
		...
		for _, v := range item {
			//content += tab(depth) + v.ColumnName + " " + v.Type + " " + v.Json + "\n"
			// 字段注释
		...
		// 添加 method 获取真实表名
		if t.realNameMethod != "" {
			content += fmt.Sprintf("func (*%s) %s() string {\n",
		...
		// 添加 method 获取clickhouse
		if t.clickHouseNameMethod != "" {
			content += fmt.Sprintf("func (*%s) %s() string {\n",
				structName, t.clickHouseNameMethod)
			content += fmt.Sprintf("%sreturn \"%s\"\n",
				tab(depth), `ods_`+tableRealName)
			content += "}\n\n"
		}

		if t.config.SeperatFile {
			if err = t.write(t.savePath+tableRealName+".go", packageName, content); err != nil {
				return err
			}
		} else {
	...
	return t.write(t.savePath, packageName, structContent)
}

func (t *Table2Struct) write(savePath, packageName string, structContent string) error {
...
func (t *Table2Struct) getColumns(table ...string) (tableColumns map[string][]column, err error) {
	...
	// sql
	var sqlStr = `SELECT COLUMN_NAME,COLUMN_KEY,DATA_TYPE,IS_NULLABLE,TABLE_NAME,COLUMN_COMMENT
	...
	for rows.Next() {
		col := column{}
		err = rows.Scan(&col.ColumnName, &col.ColumnKey, &col.Type, &col.Nullable, &col.TableName, &col.ColumnComment)
		...
		for _, cb := range t.tagCallBacks {
			col.Tag = cb(col.ColumnName, col.ColumnKey == "PRI")
		}
