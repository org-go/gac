...
/**
 * $(rewriteRouter)
 * @Description: 解决swagger路由集成覆盖问题
 * @receiver b
 * @param relativePath
 * @return string
 */
func (b *_Base) rewriteRouter(relativePath string) string {
	words := strings.Split(strings.TrimLeft(relativePath, `/`), `/`)
	if len(words) > 1 {
		switch words[0] {
		case `provider`, `platform`, `globe`, `open`:
			relativePath = `/` + strings.Join(words[2:], `/`)
		}
	}
	return relativePath
}
...
func (b *_Base) registerHandlerObj(router gin.IRouter, httpMethod []string, relativePath, methodName string, tvl, obj reflect.Value) error {
	...
	//fixme eros
	//relativePath = b.rewriteRouter(relativePath)
