...
type Tree struct {
	...
	Pk              int         `json:"pk"`               //节点Id
	ParentPk        int         `json:"parent_pk"`        //节点父级Id
...
func (nodes INodes) Less(i, j int) bool {
	return nodes[i].GetPk() < nodes[j].GetPk()
}
...
func recursiveFindRelationNode(nodeMap map[int]INode, allNodes []INode, node INode, t int) {
	nodeMap[node.GetPk()] = node
	for _, v := range allNodes {
		if _, ok := nodeMap[v.GetPk()]; ok {
			continue
		}
		// 查找父节点
		if t == 0 || t == 1 {
			if node.GetParentPk() == v.GetPk() {
				nodeMap[v.GetPk()] = v
				if v.IsRoot() {
					// 是顶层根节点时，不再进行递归
					continue
